...
exports.calculateFee = (req,res)=>{
  ...
  const ticket = tickets.find(t=>t.vehicleNumber.trim().toUpperCase()===plate && !t.exitTime);
...
exports.getRecentPayments = (req,res)=>{
  ...
  const isPrivileged = sessionUser && (sessionUser.role === 'admin' || sessionUser.role === 'operator');

  const paid = tickets.filter(t => {
    if (!t.paid) return false;
    if (!sessionUser || isPrivileged) return true;
    return t.vehicleNumber === sessionUser.license_plate;
  });

  const payments = paid.map(t=>({
