...
exports.enterParking = (req, res) => {
  ...
  freeSlot.vehicle = normalizedPlate;
...
exports.exitParking = (req, res) => {
  ...
  if (ticketId) {
    ...
    ticket = tickets.find(t => t.vehicleNumber.trim().toUpperCase() === vehicleNumber.trim().toUpperCase() && !t.exitTime);
  }

  if (!ticket || ticket.exitTime)
    return res.status(400).json({ success: false, message: 'No active ticket found for this plate.' });
